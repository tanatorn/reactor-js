...
var init = function init(args) {
  ...
  question('What is the name of your website? \n').then(function (websiteName) {
    ...
    return (0, _helper.createCustomIndex)(configuration, targetDirectory);
  }).then(function () {
    return (0, _helper.createReactorConfig)(configuration, targetDirectory);
  }).then(function () {
    var message = 'Successfully created ' + projectName + ', to start developing,\n      run \'cd ' + projectName + ' && npm install\'.\n      To start developing, use \'npm start\'. When you are ready to generate your site, ' + 'use \'npm run build\'';
    console.log(message);
  }).catch(function (err) {
    if (err.code === 'EEXIST') {
